    ...
    <div className={clsx(
      ...
      <div className="flex items-start justify-between mb-3">
        ...
        <div className={clsx(
          ...
          isFull ? 'text-red-400' : 'text-white/40',
        )}>
      ...
      <div className="flex items-center gap-2 mb-4">
        <div className="flex -space-x-2">
          {room.participants.slice(0, 4).map((p, i) => (
            <div
              key={p.id}
              style={{ zIndex: 4 - i }}
              className="w-6 h-6 rounded-full bg-gradient-to-br from-purple-600 to-pink-600 border-2 border-[#111118] flex items-center justify-center text-[9px] font-bold text-white"
            >
        ...
        <span className="text-white/30 text-xs">
          hosted by <span className="text-white/50">{room.hostUsername}</span>
        </span>
      </div>
      ...
      {room.tags.length > 0 && (
        <div className="flex flex-wrap gap-1 mb-4">
          {room.tags.slice(0, 3).map(tag => (
            <span key={tag} className="text-xs text-white/30 bg-white/5 rounded-full px-2 py-0.5">
              #{tag}
            </span>
          ))}
        </div>
...
export function Lobby({ rooms, onJoinRoom, onCreateRoom }: LobbyProps) {
