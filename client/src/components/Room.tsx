...
export function Room({
  ...
  const handleWebRTCOffer = useCallback(
    (data: { fromId: string; offer: RTCSessionDescriptionInit }) => {
      webRTC.handleOffer(data.fromId, data.offer);
    },
    [webRTC]
  );

  const handleWebRTCAnswer = useCallback(
    (data: { fromId: string; answer: RTCSessionDescriptionInit }) => {
      webRTC.handleAnswer(data.fromId, data.answer);
    },
    [webRTC]
  );
  ...
  const handleIceCandidate = useCallback(
    (data: { fromId: string; candidate: RTCIceCandidateInit }) => {
      webRTC.handleIceCandidate(data.fromId, data.candidate);
    },
    [webRTC]
  );

  const handleUserJoinedRTC = useCallback(
    (data: { participant: { id: string } }) => {
      webRTC.handleUserJoined(data.participant.id);
    },
    [webRTC]
  );

  const handleUserLeftRTC = useCallback(
    (data: { userId: string }) => {
  ...
  useEffect(() => {
    socket.on('webrtc-offer', handleWebRTCOffer as (...args: unknown[]) => void);
    socket.on('webrtc-answer', handleWebRTCAnswer as (...args: unknown[]) => void);
    socket.on('webrtc-ice-candidate', handleIceCandidate as (...args: unknown[]) => void);
    socket.on('user-joined', handleUserJoinedRTC as (...args: unknown[]) => void);
    socket.on('user-left', handleUserLeftRTC as (...args: unknown[]) => void);

    return () => {
      socket.off('webrtc-offer', handleWebRTCOffer as (...args: unknown[]) => void);
      socket.off('webrtc-answer', handleWebRTCAnswer as (...args: unknown[]) => void);
      socket.off('webrtc-ice-candidate', handleIceCandidate as (...args: unknown[]) => void);
      socket.off('user-joined', handleUserJoinedRTC as (...args: unknown[]) => void);
      socket.off('user-left', handleUserLeftRTC as (...args: unknown[]) => void);
    };
  }, [socket, handleWebRTCOffer, handleWebRTCAnswer, handleIceCandidate, handleUserJoinedRTC, handleUserLeftRTC]);
    ...
    <div className="h-[calc(100vh-56px)] flex flex-col">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-0.5">
            <span className={clsx(
              ...
              CATEGORY_COLORS[currentRoom.category],
            )}>
      ...
      {hasAudioPermission === false && (
        <div className="mx-4 mt-3 flex items-center gap-2 bg-yellow-500/10 border border-yellow-500/20 rounded-xl px-4 py-2.5 text-yellow-300 text-sm">
          ...
          <span>Microphone access denied. Voice chat unavailable. Allow mic in browser settings to enable.</span>
        </div>
      )}
